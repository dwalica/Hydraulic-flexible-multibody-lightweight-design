...
plot(t, F1, 'LineWidth',1.5, 'Color', colors(1,:));
plot(t, F2, 'LineWidth',1.5, 'Color', colors(2,:));
...
axInset = axes('Position',[0.175 0.20 0.20 0.23]); hold(axInset,'on');
...
% plot zoomed data
plot(axInset, t(idx), F1(idx), 'LineWidth',1.2, 'Color', colors(1,:));
plot(axInset, t(idx), F2(idx), 'LineWidth',1.2, 'Color', colors(2,:));
xlim(axInset, [t0 t1]); ylim(axInset, [yMinZ yMaxZ]);

% inset ticks fewer on x
nTicksInset = 3;                                    % choose how many major x ticks you want
xTicksInset = linspace(t0, t1, nTicksInset);
set(axInset, 'XTick', xTicksInset, 'XMinorTick','off');   % no minor x ticks
...
% Optional export
print -depsc2 ForceComparison.eps
